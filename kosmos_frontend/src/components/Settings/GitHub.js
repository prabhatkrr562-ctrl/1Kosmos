...
export default function GitHub() {
    ...
    return <>
        ...
        <article className="settings-panel settings-repository-panel">
            <div className="settings-repo-header">
                <PanelHeading icon="github" kicker="Source control" title="GitHub Repository" />
                <button className="settings-refresh" type="button" onClick={() => loadGitStatus()} disabled={git.loading || busy} title="Refresh status">Refresh</button>
            </div>
            {git.loading && !git.data ? <div className="settings-skeleton"><span /><span /><span /></div> : git.data ? <>
                ...
                <div className={`settings-worktree${git.data.hasChanges ? ' settings-worktree-pending' : ''}`} title={git.data.changedFiles?.join('\n') || 'Working tree is clean'}><span className="settings-worktree-icon">{git.data.hasChanges ? '!' : 'ok'}</span><div><strong>{changeLabel}</strong><span>{git.data.hasChanges ? 'Push changes before merging or restoring.' : `Ready to sync with ${git.data.mainBranch || 'main'}.`}</span></div></div>
        ...
        <ActionPanel kicker="Deployment" title="Sync changes" badge="Safe actions" description="Publish local work to a generated branch, then merge the latest branch into the stable version.">
            <ActionButton primary icon="Up" title={git.action === 'push' ? 'Pushing changes...' : 'Push changes'} detail="Create a new remote branch" disabled={blocked || !git.data?.hasChanges} onClick={() => runGitAction('push')} />
            <ActionButton icon="Sync" title={git.action === 'pull' ? 'Merging branch...' : 'Merge to stable'} detail="Promote the latest branch" disabled={blocked || !git.data?.latestBranch || git.data?.hasChanges} onClick={() => runGitAction('pull')} />
        </ActionPanel>
        ...
        <ActionPanel recovery kicker="Version control" title="Recovery" badge="Use with care" description="Return to the previous stable version locally, or publish that rollback to GitHub.">
            <ActionButton icon="Back" title={git.action === 'restore-previous' ? 'Restoring locally...' : 'Restore locally'} detail="GitHub remains unchanged" disabled={blocked || !git.data?.canRestorePrevious || git.data?.hasChanges} onClick={() => runGitAction('restore-previous')} />
            <ActionButton danger icon="Back" title={git.action === 'revert-previous' ? 'Publishing rollback...' : 'Rollback GitHub'} detail="Create a revert on main" disabled={blocked || !git.data?.canRestorePrevious || git.data?.hasChanges} onClick={() => runGitAction('revert-previous')} />
        </ActionPanel>
    </>;
}
