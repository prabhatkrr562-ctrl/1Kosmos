...
def git_pull(request):
    ...
    try:
        ...
        local_main_exists = _local_branch_exists(MAIN_BRANCH)

        if local_main_exists:
            if _current_branch() != MAIN_BRANCH:
                _git("switch", MAIN_BRANCH)
            if remote_main_exists:
                _git("pull", "--ff-only", REMOTE_NAME, MAIN_BRANCH)
        elif remote_main_exists:
            _git("fetch", REMOTE_NAME, MAIN_BRANCH)
            _git("switch", "-c", MAIN_BRANCH, "--track", f"{REMOTE_NAME}/{MAIN_BRANCH}")
        else:
            # A brand-new GitHub repository has no main ref yet. Start main at
            # the generated branch, then publish it below.
            _git("switch", "-c", MAIN_BRANCH, latest_branch)
        ...
        if merge.returncode:
            ...
            raise GitError(merge.stderr.strip() or merge.stdout.strip() or "The merge could not be completed.")

        if remote_main_exists:
            _git("push", REMOTE_NAME, MAIN_BRANCH)
        else:
            _git("push", "--set-upstream", REMOTE_NAME, MAIN_BRANCH)
        return JsonResponse({
